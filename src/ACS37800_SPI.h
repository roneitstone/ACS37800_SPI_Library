...
#ifndef ACS37800_SPI_h
#define ACS37800_SPI_h
...
//I used my values of  sense resistance for voltage measurement (Ohms)
const float ACS37800_DEFAULT_SENSE_RES = 4900;

//I used my values of voltage-divider resistance for voltage measurement (Ohms)
const float ACS37800_DEFAULT_DIVIDER_RES = 4009000;
...
const float ACS37800_DEFAULT_CURRENT_RANGE = 15; // mine acs37800 version

typedef enum {
  ACS37800_SUCCESS = 0,
  ACS37800_ERR_I2C_ERROR,
  ACS37800_ERR_REGISTER_READ_MODIFY_WRITE_FAILURE,
  ACS37800_ERR_SPI_ERROR  // Adicionando o erro SPI
} ACS37800ERR;
...
class ACS37800
{
  ...
  public:
    ...
    //ACS37800KMACTR-090B3-I2C is a 90.0 Amp part
    void beginSPI(uint8_t csPin = 5); // Function added for spi communication

    //Debugging
    void enableDebugging(Stream &debugPort = Serial); //Disabled.

    //Basic methods for accessing registers
    ACS37800ERR readRegister(uint32_t *data, uint8_t regAddress); // Modified for spi communication
    ACS37800ERR writeRegister(uint32_t value, uint8_t regAddress); // Modified for spi communication
    ...
    ACS37800ERR readPowerActiveSec(float *pActive); // Read volatile register 0x28. Return the pactive averaged for 1 second.
    
    ACS37800ERR readOnlyPowerFactor(float *pFactor); // Read volatile register 0x22. Return only the  power factor.
    ...
    ACS37800ERR readErrorFlags(ACS37800_REGISTER_2D_t *errorFlags); // Disabled.
  ...
  private:
    uint8_t _csPin;
    // Debug
    Stream *_debugPort; // Disabled.
    bool _printDebug = false; // Disabled.
